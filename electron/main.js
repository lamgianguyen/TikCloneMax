...
const AUTH_HOST = process.env.TIKFINITY_AUTH_HOST || "https://tikpr0.com";
...
app.on("quit", (_e, code) => {
  console.log("[LIFECYCLE] quit code=" + code);
});
...
function freeOurPorts() {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      seenPids.add(pid);
      try {
        spawnSync("taskkill.exe", ["/F", "/PID", String(pid)], {
          timeout: 3000,
        });
...
function startBackend() {
  ...
  try {
    ...
    child.stdout.on("data", (d) => {
      const m = d.toString();
      try {
        backendLogStream && backendLogStream.write(m);
      } catch {
        /* ignore */
      }
    ...
    child.stderr.on("data", (d) => {
      const m = d.toString();
      try {
        backendLogStream && backendLogStream.write("[ERR] " + m);
      } catch {
        /* ignore */
      }
      const trimmed = m.trim();
...
function startBridgeToDapiRelay() {
  ...
  try {
    ...
    console.warn(
      "[Relay] socket.io-client not installed — bridge→DAPI relay disabled. Run `npm install` inside electron/.",
    );
    return;
  }
  ...
  const connect = () => {
    ...
    sock.on("connect", () => {
      console.log(
        "[Relay] Bridge→DAPI Socket.IO client connected (id=" + sock.id + ")",
      );
...
async function decideAuthEntrypoint() {
  ...
  if (!cached) {
    ...
    if (ttSession && ttSession.sessionId) {
      console.log(
        "[Auth] No Serial Key, but TikTok session exists — allowing offline entry",
      );
  ...
  if (!remote.reachable) {
    ...
    if (ttSession && ttSession.sessionId) {
      console.log(
        "[Auth] Server unreachable — allowing offline entry with saved TikTok session",
      );
      return {
        allow: true,
        offline: true,
        reason: "SERVER_UNREACHABLE_OFFLINE",
      };
    }
  ...
  authReCheckTimer = setInterval(async () => {
    ...
    if (currentAuth.expiresAt) {
      ...
      if (Number.isFinite(exp) && exp <= Date.now()) {
        console.log(
          "[Auth] Key expired during session — kicking back to login.",
        );
        showExpiryNotice(
          "⏰ Serial Key đã hết hạn",
    ...
    try {
      ...
      if (remote.reachable && !remote.valid) {
        console.log(
          `[Auth] Server says invalid (${remote.reason}) — kicking back to login.`,
        );
        showExpiryNotice(
          "🚫 Serial Key không còn hợp lệ",
      ...
      /* network blip — ignore until next tick */
    }
...
function tfsRequest(method, path, body, extraHeaders) {
  return new Promise((resolve, reject) => {
    ...
    const isLocalhostHost =
      /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?(\/|$)/i.test(AUTH_HOST);
    const opts = {
      method,
      headers: {
  ...
  loginWindow.once("ready-to-show", () => {
    ...
    if (reason && reason !== "NO_TOKEN") {
      ...
      if (msg && loginWindow && !loginWindow.isDestroyed()) {
        loginWindow.webContents.send("auth:state", {
          kind: "error",
          message: msg,
        });
      }
...
function describeAuthReason(reason) {
  switch (reason) {
    case "EXPIRED":
      return "⏰ Serial Key đã hết hạn. Vui lòng gia hạn hoặc nhập key mới.";
    case "INVALID":
      return "Serial Key không hợp lệ. Vui lòng nhập lại.";
    case "SERVER_UNREACHABLE":
      return "🌐 Không kết nối được server xác thực. Hãy kiểm tra mạng / chạy TikfinityServer rồi thử lại.";
    case "OFFLINE_EXPIRED":
      return "Đã hơn 7 ngày offline. Cần kết nối server để xác thực Serial Key.";
    case "DISABLED":
      return "🚫 Serial Key đã bị khoá. Liên hệ admin để mở lại.";
    case "NOT_FOUND":
      return "Serial Key không tồn tại trong hệ thống.";
    case "NOT_ACTIVATED":
      return "Serial Key chưa được kích hoạt (chưa có chủ sở hữu).";
    case "INVALID_KEY_CODE":
      return "Key Code không khớp với Serial Key.";
    case "LOGOUT":
      return null; // user-initiated, không cần thông báo
    default:
      return null;
  }
}
...
ipcMain.handle("auth:validate-key", async (_evt, payload) => {
  ...
  try {
    ...
    if (!res || !res.ok) {
      return {
        ok: false,
        message: `Server trả lỗi (HTTP ${res?.status || "unknown"}).`,
      };
    }
    const body = res.body || {};
    if (!body.valid) {
      return {
        ...
        message:
          body.message ||
          describeAuthReason(body.reason) ||
          "Key không hợp lệ.",
      };
    }
    currentAuth = authStore.save({
...
ipcMain.handle("auth:login-user", async (_evt, payload) => {
  ...
  if (!username || !password)
    return { ok: false, message: "Nhập đầy đủ tài khoản và mật khẩu." };

  try {
    const res = await tfsRequest("POST", "/api/auth/login", {
      username,
      password,
    });
    ...
    if (!res.ok) {
      return {
        ok: false,
        message:
          res.body?.message || `Đăng nhập thất bại (HTTP ${res.status}).`,
      };
    }
    const body = res.body || {};
  ...
  refreshInitialApiState()
    .then(() => {
      if (mainWindow && !mainWindow.isDestroyed()) {
        ...
        if (seed) {
          try {
            mainWindow.webContents.send("auth:seed-updated", seed);
          } catch {
            /* renderer gone */
          }
        }
    ...
    .catch(() => {
      /* swallow — preload will fall back to stale seed */
    });
});
...
function performLogout(options = {}) {
  ...
  try {
    ...
    console.warn(
      "[Auth] clear TT session on logout failed:",
  ...
  if (relaunchApp) {
    ...
    try {
      ...
      isQuitting = false;
      console.warn(
        "[Auth] app.relaunch failed, fallback to in-app login window:",
        err && err.message ? err.message : err,
      );
...
function finishLogin() {
  ...
  if (!mainWindow || mainWindow.isDestroyed()) {
    ...
    prepareRendererAuthSeed()
      .catch((err) =>
        console.warn("[tfs] renderer auth refresh failed:", err.message),
      )
      .finally(() => {
...
function configureSession() {
  ...
  const TF_CDN_HOSTS = new Set([
    "tikfinity-assets.b-cdn.net",
    "assets.tikfinity.com",
  ]);
  ...
  function buildTtsRedirect(u) {
    ...
    return (
      `${TTS_GENERATE_BASE}?voice=${encodeURIComponent(voice)}` +
  ...
  sess.webRequest.onBeforeRequest(
    {
      urls: [
        ...
        "https://*.tiktokcdn.com/*",
        "http://*.tiktokcdn.com/*",
        "https://tikfinity-assets.b-cdn.net/*",
        "https://assets.tikfinity.com/*",
        // TTS non-AI voice endpoints (see buildTtsRedirect above)
        "https://www.google.com/speech-api/*",
        "http://www.google.com/speech-api/*",
        "https://tikfinity-tts-api.zerody.one/*",
        "http://tikfinity-tts-api.zerody.one/*",
      ],
    },
    (details, callback) => {
      let u;
      ...
      if (TIKTOK_HOST_RE.test(host)) {
        return callback({
          redirectURL: `${TIKTOK_CACHE_BASE}/${host}${u.pathname}${u.search}`,
        });
      }
      // (c) TikFinity asset CDNs → local disk-cached proxy
      if (TF_CDN_HOSTS.has(host)) {
        return callback({
          redirectURL: `${TF_CDN_BASE}/${host}${u.pathname}${u.search}`,
        });
      }
      // (c2) TTS non-AI voice (Google revoked / zerody proxy) → real TikTok TTS
      if (
        (host === TTS_GOOGLE_HOST && u.pathname.startsWith("/speech-api/")) ||
  ...
  console.log(
    "[Electron] Merged intercept installed (telemetry-block + tiktokcdn→" +
      TIKTOK_CACHE_BASE +
      " + tf-cdn→" +
      TF_CDN_BASE +
      " + tts→" +
  ...
  sess.webRequest.onHeadersReceived(
    ...
    (details, callback) => {
      ...
      if (
        details.url.includes("clienttoken.spotify.com") ||
  ...
  sess.webRequest.onBeforeSendHeaders(
    ...
    (details, callback) => {
      ...
      if (
        details.url.startsWith("https://api.spotify.com/") ||
      ...
      res.on("end", () => {
        try {
          ...
          cachedInitialApiState = {
            channelId:
              parsed?.channel?.ChannelId || parsed?.channel?.channelId || null,
            profileId:
              parsed?.channel?.ProfileId || parsed?.channel?.profileId || null,
            channelName:
              parsed?.channelName || parsed?.channel?.ChannelName || null,
...
function createMainWindow() {
  ...
  function startUiHealthWatchdog() {
    ...
    _watchdogTimer = setInterval(async () => {
      ...
      try {
        ...
        console.warn(
          `[UI-Health] FAIL #${_watchdogFailCount}/${WATCHDOG_FAILS_BEFORE_RELOAD}:`,
          r,
        );
        if (_watchdogFailCount >= WATCHDOG_FAILS_BEFORE_RELOAD) {
          ...
          if (since < WATCHDOG_RELOAD_COOLDOWN_MS) {
            console.warn(
              `[UI-Health] auto-reload suppressed (last was ${Math.round(since / 1000)}s ago)`,
            );
            _watchdogFailCount = 0;
            return;
          }
          ...
          try {
            ...
            console.warn("[UI-Health] reload threw:", e.message);
          }
        }
      } catch (err) {
        ...
        if (
          err &&
          err.message &&
          err.message.indexOf("Render frame was disposed") === -1
        ) {
    ...
    console.log(
      `[UI-Health] watchdog active (every ${WATCHDOG_INTERVAL_MS}ms, dev only)`,
    );
  }

  const _maxWaitTimer = setTimeout(() => {
    console.warn(
      `[Main-Boot] hard timeout ${MAX_WAIT_MS}ms — showing main window anyway`,
    );
    showMainWindowOnce();
  }, MAX_WAIT_MS);
  ...
  const rendererErrorLog = path.join(
    app.getPath("userData"),
    "renderer-errors.log",
  );
  function appendRendererLog(line) {
    try {
      const stat = fs.existsSync(rendererErrorLog)
        ? fs.statSync(rendererErrorLog)
        : null;
      if (stat && stat.size > 2 * 1024 * 1024) {
        try {
          fs.renameSync(rendererErrorLog, rendererErrorLog + ".old");
        } catch {
          /* ignore */
        }
      }
      ...
      /* don't crash the main process on log write failure */
    }
  }
  ...
  mainWindow.webContents.on(
    ...
    (_evt, level, message, line, sourceId) => {
      ...
      appendRendererLog(
        `${new Date().toISOString()} ${prefix}${loc}\n  ${trimmed}`,
      );
    },
  );
  ...
  mainWindow.webContents.on("will-navigate", (event, url) => {
    ...
    if (gap > SETTLE_GAP_MS) {
      if (_chainBlocked)
        console.log(`[Main-Guard] Chain released after ${gap}ms settle gap`);
      _chainCount = 0;
      _chainBlocked = false;
    }
    ...
    if (_initialBootPhase) {
      ...
      if (_bootNavCount <= MAX_BOOT_NAVS) {
        console.log(
          `[InitialBoot] nav during boot (#${_bootNavCount}/${MAX_BOOT_NAVS}): ${url}`,
        );
        _chainCount = 0;
        return;
      }
      ...
      console.warn(
        `[InitialBoot] exceeded ${MAX_BOOT_NAVS} navs — ending boot phase early`,
      );
    ...
    if (_chainBlocked) {
      console.warn(
        `[Main-Guard] BLOCKED reload (chain stuck, awaiting ${SETTLE_GAP_MS}ms settle gap, gap so far ${gap}ms) url=${url}`,
      );
      event.preventDefault();
      return;
    }
    ...
    if (_chainCount > CHAIN_LIMIT) {
      _chainBlocked = true;
      console.warn(
        `[Main-Guard] CHAIN OVERFLOW (>${CHAIN_LIMIT} navs without settle). Blocking until ${SETTLE_GAP_MS}ms gap. url=${url}`,
      );
      event.preventDefault();
    }
  });
  ...
  mainWindow.webContents.on(
    ...
    (_e, url, isInPlace, isMainFrame) => {
      ...
      if (isBackendMainUrl(url)) {
        console.log(
          `[Main-Diag] mainFrame nav (chain=${_chainCount}) url=${url}`,
        );
      } else {
  ...
  mainWindow.webContents.on("did-fail-load", (_e, code, desc, url) => {
    console.error(
      `[Main-Diag] did-fail-load code=${code} desc=${desc} url=${url}`,
    );
  });
  ...
  mainWindow.webContents.on("will-prevent-unload", (_event) => {
    console.warn(
      "[Main-Guard] will-prevent-unload — page frozen by reloadGuard kill-switch (renderer beforeunload preventDefault honored by Electron default)",
    );
  });
  ...
  prepareRendererAuthSeed()
    .catch((err) =>
      console.warn("[tfs] renderer auth seed prepare failed:", err.message),
    )
    .finally(() => {
      if (!mainWindow || mainWindow.isDestroyed()) return;
...
function applyAuthOverlay() {
  ...
  const persist = `
    ...
    `;
  mainWindow.webContents.executeJavaScript(persist, true).catch(() => {
    /* page navigated mid-inject */
  });
}
...
function handleWindowOpen(details) {
    ...
    if (!signInStatus || !signInStatus.signedIn) {
      setImmediate(() => {
        ...
        tiktokSignin.openSignIn(parentWindow).catch((err) => {
          console.error(
            "[TikTok Login] Failed to open sign-in from tfbridge:",
            err.message,
          );
        });
      });
  ...
  if (isTikTokUrl && wantsTikTokLogin) {
    setImmediate(() => {
      ...
      tiktokSignin.openSignIn(parentWindow).catch((err) => {
        console.error(
          "[TikTok Login] Failed to open dedicated sign-in window:",
          err.message,
        );
      });
    });
  ...
  if (
    url.startsWith(BACKEND_URL + "/widget/") ||
      ...
      if (isLive) {
        ...
        if (powerSaveBlockerId === null) {
          try {
            ...
            console.warn("[powerSaveBlocker] start:", e.message);
          }
        }
...
function execPsCommand(command, returnResult) {
  ...
  const { stdout, stderr, status } = spawnSync("powershell", [
    "-NoProfile",
    "-Command",
    command,
  ]);
