...
| **QA tự động (chạy TRƯỚC/SAU mọi đợt sửa + sau bundle update)** | **[qa/README.md](qa/README.md)** + `npm run qa` | Harness tự duy trì: L1 API / L2 socket-relay / L3 widget / perf (ram/cpu/lag/crash) + L4 gate-drift. Tự ghi block `QA-AUTO` vào TEST_STATUS + `QA-AUTO-FAILURES` vào FIXLOG. Bundle mới → **[qa/BUNDLE_UPDATE.md](qa/BUNDLE_UPDATE.md)** (gate-health báo Gate nào trôi). Đội ngũ: [.codex/team/qa-team.md](.codex/team/qa-team.md) + Workflow `qa-sweep`. Thêm test = thêm 1 dòng `qa/registry.js`, KHÔNG viết lại code |
